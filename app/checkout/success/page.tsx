...
import { CheckCircle2, Calendar, Clock, MapPin, Ticket, ArrowRight, Home, Download, Share2, Loader2 } from 'lucide-react'
import { Header } from '@/components/header'
import { Footer } from '@/components/footer'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'

// 1. CREAMOS EL COMPONENTE QUE TIENE TODA LA LÓGICA
function SuccessContent() {
  ...
  return (
    <>
      <div className="flex-1 flex items-center justify-center py-20 px-4 print:hidden">
        <div className="max-w-xl w-full">
          ...
          >
            <div className="bg-card border border-border border-b-0 rounded-t-[2.5rem] p-8 shadow-sm">
              <div className="flex justify-between items-start mb-8">
                ...
                <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 font-bold px-3 py-1">Pagado</Badge>
              </div>
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="px-4 text-primary"><ArrowRight className="size-6" /></div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-8">
            ...
            <Button variant="outline" className="rounded-2xl h-14 font-bold border-border bg-card"><Share2 className="size-4 mr-2" /> Compartir</Button>
      ...
      {/* TICKET DE IMPRESIÓN */}
      <div className="hidden print:flex flex-col items-center justify-center w-full min-h-screen bg-white text-black">
        <div className="w-[340px] border-[2px] border-gray-200 rounded-3xl p-6 bg-white shadow-none">
          ...
          <div className="flex items-center justify-between mb-8 bg-gray-50 p-4 rounded-2xl border border-gray-100">
            <div className="text-center"><p className="text-4xl font-black text-gray-900">{shortOrigin}</p></div>
            <ArrowRight className="size-6 text-gray-300" />
            <div className="text-center"><p className="text-4xl font-black text-gray-900">{shortDest}</p></div>
          </div>
          <div className="grid grid-cols-2 gap-y-6 mb-8 border-y border-dashed border-gray-300 py-6">
            <div><p className="text-[10px] text-gray-400 uppercase font-bold mb-1 tracking-widest">Fecha</p><p className="text-sm font-black text-gray-900">{date}</p></div>
            <div><p className="text-[10px] text-gray-400 uppercase font-bold mb-1 tracking-widest">Hora</p><p className="text-sm font-black text-gray-900">{time}</p></div>
            <div><p className="text-[10px] text-gray-400 uppercase font-bold mb-1 tracking-widest">Asiento</p><p className="text-sm font-black text-gray-900">{seats.join(', ')}</p></div>
            <div><p className="text-[10px] text-gray-400 uppercase font-bold mb-1 tracking-widest">Autobús</p><p className="text-sm font-black text-gray-900">{busNumber}</p></div>
          </div>
...
// 2. EL COMPONENTE PRINCIPAL QUE EXPORTA LA PÁGINA
    ...
    <main className="min-h-screen bg-muted/30 flex flex-col">
      ...
      <Suspense fallback={
        <div className="flex-1 flex flex-col items-center justify-center py-32">
          ...
          <h2 className="text-xl font-bold">Cargando tu ticket...</h2>
        </div>
      }>
        <SuccessContent />
      </Suspense>
